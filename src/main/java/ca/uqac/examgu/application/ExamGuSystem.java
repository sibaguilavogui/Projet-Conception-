...
public class ExamGuSystem {
    ...
    private void assignCode(Utilisateur u) {
        ...
        // 2) Code permanent (ETU/ENS seulement) si on a les infos
    ...
    public Tentative trouverTentative(UUID id) {
        return tentatives.get(id);
    }

    // ✅ utile au prof pour voir les tentatives d’un examen (correction)
    public List<Tentative> listerTentativesParExamen(UUID examenId) {
        ...
        for (Tentative t : tentatives.values()) {
            if (t.getExamen() != null && examenId.equals(t.getExamen().getId())) {
                res.add(t);
            }
        }
        return res;
    ...
    public Collection<Utilisateur> listerUtilisateurs() {
        return Collections.unmodifiableCollection(utilisateurs.values());
    }

    public Collection<Examen> listerExamens() {
        return Collections.unmodifiableCollection(examens.values());
    }

    public Collection<Tentative> listerTentatives() {
        return Collections.unmodifiableCollection(tentatives.values());
    }
    ...
    public void planifierEtOuvrirExamen(Utilisateur acteurEnseignant, UUID examenId,
    ...
    public void fermerExamen(Utilisateur acteurEnseignant, UUID examenId) {
        requireRole(acteurEnseignant, Role.ENSEIGNANT);
        Examen ex = getExamenOrThrow(examenId);
        ex.fermer();
        log.info(() -> String.format("[FERMER_EXAMEN] examen=%s fermé par %s",
                ex.getCode(), acteurEnseignant.getCode()));
    }
            ...
            for (Question q : ex.getQuestions()) {
                ...
                for (ReponseDonnee rd : t.getReponses()) {
                    if (rd != null && rd.getQuestion() != null &&
                            q.getId().equals(rd.getQuestion().getId())) {
                        rep = rd.getContenu();
                        break;
                    }
                }
    ...
    // ---------------- LISTE D'EXAMENS PAR ENSEIGNANT ----------------

    public List<Examen> listerExamensPourEnseignant(Utilisateur acteurEnseignant) {
        requireRole(acteurEnseignant, Role.ENSEIGNANT);
        UUID ensId = acteurEnseignant.getId();
        List<Examen> res = new ArrayList<>();
        for (Examen ex : examens.values()) {
            if (ex.getCreateurId() != null && ex.getCreateurId().equals(ensId)) {
                res.add(ex);
            }
        }
        return res;
    ...
    public void sauvegarderReponse(Utilisateur acteurEtudiant, UUID tentativeId,
                                   UUID questionId, String contenu, LocalDateTime now) {
        requireRole(acteurEtudiant, Role.ETUDIANT);
        Tentative t = getTentativeOrThrow(tentativeId);
    ...
    private String codePermanentUnique(String prenom, String nom, LocalDate dn) {
        ...
        return base + UUID.randomUUID().toString().substring(0, 2)
